        ...
        try {
            ...
            if (processed_file) {
                ...
                if (target_channel?.isText()) {
                    processed_file.startsWith('https') ? await target_channel.send(processed_file) : await target_channel.send(`https://raws.reaperscans.com/${processed_file}`)
